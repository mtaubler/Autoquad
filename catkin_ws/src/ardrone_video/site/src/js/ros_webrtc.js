...
ros_webrtc.CallKMS = Backbone.Model.extend({
  ...
  initialize: function () {
    ...
    var args = {
      ...
      file_uri: 'file:///home/rthechief/Autoquad/recordings/' + getFormattedDate() + '.webm' // file to be stored in media server
    };
    ...
    kurentoClient(args.ws_uri, function(error, client) {
      ...
      console.info("Got KurentoClient");

      client.create('MediaPipeline', function(error, pipeline) {
        if (error) return onError(error);

        console.info("Got MediaPipeline");
        ...
        pipeline.create(elements, function(error, elements){
          ...
          console.info("Got Elements");

          webRtc.on('OnIceCandidate', function(event) {
            var candidate = event.candidate;

            console.info("Remote candidate:", candidate);

            that.get('signaling').addIceCandidate(that.get('id'), that.get('peer_id'), candidate);
            /*that.get('signaling').addIceCandidate(that.get('id'), that.get('peer_id'), {
              candidate: candidate.candidate,
              sdpMid: candidate.sdpMid,
              sdpMLineIndex: candidate.sdpMLineIndex
            });*/
          });
          webRtc.on('OnIceGatheringDone', function(event) {
            console.info("Ice gathering done!", event);

            that.get('signaling').addIceCandidate(that.get('id'), that.get('peer_id'), null);
          });
  ...
  startRecording: function () {
    var that = this;
    if(this.kurento) {
      console.info("attempting record");
      this.kurento.recorder.record(this.__withError(function() {
        console.info("record");
      }));
    }
  },
  ...
  _kurentoConnected: function () {
    console.info("Kurento connected");
    this.trigger("kurento", this);
  },
  ...
  _drainRemoteIceCandidates: function () {
    ...
    while (this.remote_ice_candidates.length !== 0) {
      ...
      this.__sendKurentoIceCandidate(ice_candidate);
    }
  },
  ...
  _pickup: function(signaling, session_id) {
    ...
    this.__withKurento(function() {
      this.kurento.webRtc.generateOffer(this.__withError(function(offer) {
        console.info('pc set local description', offer);
        this.get('signaling').setSessionDescription(this.get('id'), this.get('peer_id'), {
          type: "offer",
          sdp: offer
        });
        this.kurento.webRtc.gatherCandidates(this.__withError());
      }));
    });
  },
  ...
  _setSessionDescription: function (signaling, session_id, offer_or_answer) {
    ...
    if(offer_or_answer.type !== 'answer') {
      console.info("peer attempted to send offer, which is unsupported");
      return;
    }

    if(this.kurento) {
      this.kurento.webRtc.processAnswer(offer_or_answer.sdp, this.__withError(function(sdp) {
        console.info('pc set remote description', offer_or_answer.sdp);
        this._drainRemoteIceCandidates();
      }));
    }
  },

  __sendKurentoIceCandidate: function(ice_candidate) {
    this.__withKurento(function() {
      var candidate = kurentoClient.getComplexType('IceCandidate')(ice_candidate);

      this.kurento.webRtc.addIceCandidate(candidate, this.__withError(function() {
          console.info( 'pc added remote ice candidate', ice_candidate);
      }));
    });
  },
  __withKurento: function(f) {
    f = f.bind(this);
    if(this.kurento)
      f();
    else
